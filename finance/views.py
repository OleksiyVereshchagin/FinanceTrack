...
import random
from .utils import get_expense_trends
...
#                 ).order_by('date')
#
#                 # Оновлений блок — обрахунок середнього + js-готове поле
#                 avg_by_category = list(
...
#                     item['avg_js'] = float(round(item['avg'], 2))
#                 context['avg_by_category'] = avg_by_category
#
...
#             )
#
#             # --- Генерація rule-based фінансових порад ---
#             advice = []
#
...
class ExpenseAnalysisView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            # if not context['short_period']:
            #     # 1. Категорія > 40% усіх витрат
            #     if expenses.exists():
            #         by_cat = expenses.values('category__name').annotate(sum=Sum('amount'))
            #         for cat in by_cat:
            #             if cat['sum'] > total_expenses * Decimal('0.4'):
            #                 advice.append(
            #                     random.choice(advice_variants['high_category_spending']).format(cat=cat['category__name'])
            #                 )
            #
            ...
            #         advice.append(random.choice(advice_variants['no_savings_activity']))
            #
            # # 5. Сплески витрат — показуємо завжди
            # if context.get('expenses_by_day'):
            #     day_values = [entry['total'] for entry in context['expenses_by_day']]
            #     if day_values:
            #         avg_day = sum(day_values) / len(day_values)
...
#                 'currency': currency,
#                 'insight': generate_insight(category.split(' (')[0], trend, change_percent, forecast, currency)
#             })
#
#     # Сортування за зміною (за абсолютним значенням)
#     return sorted(results, key=lambda x: abs(x['change_percent']), reverse=True)
#
# def generate_insight(category, trend, percent, forecast, currency):
#     """Генерує текстові рекомендації"""
#     trend_map = {
#         'increase': f"Зростання витрат на {category} на {abs(percent)}%",
#         'decrease': f"Зниження витрат на {category} на {abs(percent)}%",
#         'stable': f"Стабільні витрати на {category}"
#     }
#     return f"{trend_map.get(trend, 'Зміна витрат')}. Прогноз на наступний місяць: {forecast:.2f} {currency}."
#
